...
export class TeachingEngineService {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          const response = await ai.models.generateContent({
            ...
            config: { responseMimeType: 'application/json', temperature: attempt === 1 ? 0.3 : 0.2 },
          });
          ...
          if (rawText && rawText.trim().length > 0) {
            try {
              ...
              console.warn(`[TeachingEngine] Structure JSON parse attempt ${attempt} failed:`, pErr);
            }
          }
        } catch (err) {
          console.warn(`[TeachingEngine] Structure generation attempt ${attempt} failed:`, err);
        }
  ...
  private async fetchSingleBoardFromAI(
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const response = await ai.models.generateContent({
          model: this.appSettings.openrouter_model || 'qwen/qwen3.7-flash',
          contents: [{ role: 'user', parts: [{ text: `${TEACHING_DIRECTOR_SYSTEM_PROMPT}\n\n${prompt}` }] }],
          config: { responseMimeType: 'application/json', temperature: attempt === 1 ? 0.35 : 0.2 },
        });
        ...
        if (rawText && rawText.trim().length > 0) {
          try {
            ...
            if (performance && (performance.title || performance.speech)) {
              break;
            }
          } catch (pErr) {
            console.warn(`[TeachingEngine] Board performance JSON parse attempt ${attempt} failed:`, pErr);
          }
        }
      } catch (err) {
        console.warn(`[TeachingEngine] fetchSingleBoardFromAI attempt ${attempt} failed:`, err);
      }
      if (attempt < 3) await new Promise((res) => setTimeout(res, attempt * 500));
    }
    ...
    if (!performance) {
      ...
      performance = buildFallbackBoardPerformance(boardPlan, this.currentStructure?.topic || 'Lesson');
    }
...
function buildFallbackTeachingStructure(topicName: string, durationMode?: LessonDurationMode): TeachingStructure {
  const duration = durationMode === '15min' ? 15 : durationMode === '60min' ? 60 : 30;
  return {
    topic: topicName || 'Academic Tutorial',
    totalEstimatedDurationMinutes: duration,
    pedagogicalStrategy: 'Step-by-step intuitive breakdown of core concepts',
    boards: [
      {
        board_number: 1,
        title: `${topicName} — Key Concepts & Overview`,
        visual_theme: 'Concept structure diagram',
        core_concept_explained: `Core principles of ${topicName}`,
        key_takeaways: ['Foundational definitions', 'Primary mechanism', 'Key applications'],
      },
      {
        board_number: 2,
        title: `${topicName} — Detailed Process & Examples`,
        visual_theme: 'Step-by-step analytical breakdown',
        core_concept_explained: `Detailed analysis of ${topicName}`,
        key_takeaways: ['Step-by-step walkthrough', 'Common patterns & formulas', 'Practical examples'],
      },
      {
        board_number: 3,
        title: `${topicName} — Summary & Checkpoint`,
        visual_theme: 'Synthesis and review summary',
        core_concept_explained: `Synthesis and problem solving for ${topicName}`,
        key_takeaways: ['Summary of core insights', 'Key takeaways review', 'Next steps'],
      },
    ],
  };
}

function buildFallbackBoardPerformance(
  boardPlan: TeachingBoardPlan,
  topicName: string
): TeachingBoardPerformance {
  const boardNum = boardPlan.board_number || 1;
  const boardTitle = boardPlan.title || `${topicName} - Board ${boardNum}`;
  const takeaways = boardPlan.key_takeaways && boardPlan.key_takeaways.length > 0
    ? boardPlan.key_takeaways
    : ['Core concept overview', 'Key principles and applications'];
  ...
  const actions: BoardAction[] = [
    {
      ...
      content: boardTitle,
      position: { x: 50, y: 15 },
      style: { fontSize: '24px', fontWeight: 'bold', color: '#38BDF8' },
    },
    ...takeaways.map((kt, idx) => ({
      id: `act_kt_${boardNum}_${idx}`,
      type: 'text' as const,
      content: `• ${kt}`,
      position: { x: 20, y: 35 + idx * 12 },
      style: { fontSize: '18px', color: '#F3F4F6' },
    })),
  ];

  return {
    board_id: boardPlan.board_id || `b_${boardNum}`,
    board_number: boardNum,
    title: boardTitle,
    speech: `Welcome to ${boardTitle}. Today we focus on ${boardPlan.core_concept_explained || topicName}. ${takeaways.join('. ')}.`,
    speech_beats: [
      {
        beat_id: `beat_${boardNum}_1`,
        text: `Welcome to ${boardTitle}`,
        board_actions: [actions[0]],
      },
    ],
    board_actions: actions,
    question: {
      question: `What is the primary takeaway of ${boardTitle}?`,
      expectedConcepts: takeaways,
      waitForAnswer: false,
    },
  };
}
