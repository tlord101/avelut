...
export const NotebookDetail: React.FC<NotebookDetailProps> = ({
  ...
  return (
    <div className="flex-1 w-full max-w-4xl mx-auto p-4 sm:p-6 pb-[calc(76px+env(safe-area-inset-bottom)+20px)] overflow-y-auto space-y-4 animate-fade-in text-slate-900 dark:text-slate-100">
      ...
      <div className="bg-white dark:bg-[#141414] border border-[#E3E9F1] dark:border-[#2A2A2A] rounded-3xl p-6 sm:p-7 shadow-xs">
        <div>
        ...
        {/* Metadata Pills */}
        <div className="flex items-center gap-2.5 flex-wrap text-xs text-[#64748B] dark:text-[#A3A3A3] pt-3 mt-3 border-t border-[#E3E9F1] dark:border-[#2A2A2A]">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#F6F6F3] dark:bg-[#1C1C1C] rounded-full border border-[#E3E9F1] dark:border-[#2A2A2A]">
            <i className="bi bi-file-earmark-text text-[#2563EB] dark:text-[#3B82F6]"></i>
            {notebook.total_pages} {notebook.total_pages === 1 ? 'Page' : 'Pages'}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#F6F6F3] dark:bg-[#1C1C1C] rounded-full border border-[#E3E9F1] dark:border-[#2A2A2A]">
            <i className="bi bi-bookmark text-[#2563EB] dark:text-[#3B82F6]"></i>
            {notebook.chapter_count} {notebook.chapter_count === 1 ? 'Chapter' : 'Chapters'}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#F6F6F3] dark:bg-[#1C1C1C] rounded-full border border-[#E3E9F1] dark:border-[#2A2A2A]">
            <i className="bi bi-hdd text-[#2563EB] dark:text-[#3B82F6]"></i>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-xs text-[#64748B] dark:text-[#A3A3A3]">Select a chapter to study</span>
        </div>
        ...
        {notebook.chapters.map((ch, idx) => {
          ...
          return (
            <div
              ...
              className="w-full flex items-center justify-between p-4 sm:p-5 bg-white dark:bg-[#141414] border border-[#E3E9F1] dark:border-[#2A2A2A] rounded-2xl hover:border-[#2563EB]/50 dark:hover:border-[#3B82F6]/50 transition-all cursor-pointer group shadow-2xs gap-3"
            >
              <div className="flex items-center gap-3.5 min-w-0">
                <div className="w-10 h-10 rounded-2xl bg-[#F6F6F3] dark:bg-[#1C1C1C] border border-[#E3E9F1] dark:border-[#2A2A2A] flex items-center justify-center text-[#0F172A] dark:text-white font-bold text-sm shrink-0 group-hover:bg-[#2563EB] dark:group-hover:bg-[#3B82F6] group-hover:text-white transition-colors">
                  {idx + 1}
                </div>
                <div className="min-w-0">
                  <h4 className="text-sm font-bold text-[#0F172A] dark:text-white truncate group-hover:text-[#2563EB] dark:group-hover:text-[#3B82F6] transition-colors">
                    {ch.title}
                  </h4>
                  <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-[#64748B] dark:text-[#A3A3A3]">
                    {visitedLabel && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-[#F1F5F9] dark:bg-[#1C1C1C] text-[#64748B] dark:text-[#A3A3A3] text-[10px] font-semibold border border-[#E3E9F1] dark:border-[#2A2A2A]">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="hidden sm:inline-block text-xs font-bold text-[#2563EB] dark:text-[#3B82F6] opacity-0 group-hover:opacity-100 transition-opacity">
                Study Chapter
              </span>
              <div className="w-8 h-8 rounded-full bg-[#F6F6F3] dark:bg-[#1C1C1C] border border-[#E3E9F1] dark:border-[#2A2A2A] flex items-center justify-center text-[#0F172A] dark:text-white group-hover:bg-[#2563EB] dark:group-hover:bg-[#3B82F6] group-hover:text-white transition-all">
      ...
      {showActionModal && selectedChapter && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6">
          ...
          {/* Modal Container */}
          <div className="relative w-full max-w-lg bg-white dark:bg-[#141414] border border-[#E3E9F1] dark:border-[#2A2A2A] rounded-3xl p-5 sm:p-7 shadow-2xl z-10 animate-fade-in space-y-5 max-h-[90vh] overflow-y-auto text-[#0F172A] dark:text-white">
            {/* Modal Header */}
            <div className="flex items-start justify-between border-b border-[#E3E9F1] dark:border-[#2A2A2A] pb-3.5">
              <div className="min-w-0 pr-2">
                <span className="text-[10px] font-bold text-[#64748B] dark:text-[#A3A3A3] uppercase tracking-wider block">
                  Choose Study Mode
                </span>
              ...
              <button
                type="button"
                onClick={() => setShowActionModal(false)}
                className="w-8 h-8 rounded-full bg-[#F6F6F3] dark:bg-[#1C1C1C] hover:bg-white dark:hover:bg-[#2A2A2A] border border-[#E3E9F1] dark:border-[#2A2A2A] flex items-center justify-center text-[#0F172A] dark:text-white transition-all cursor-pointer shrink-0"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              ...
              <button
                ...
                className="flex items-center gap-3 p-3.5 bg-[#F6F6F3] dark:bg-[#1C1C1C] hover:bg-[#F1F5F9] dark:hover:bg-[#2A2A2A] border border-[#E3E9F1] dark:border-[#2A2A2A] hover:border-[#2563EB]/50 dark:hover:border-[#3B82F6]/50 rounded-2xl text-left transition-all cursor-pointer group"
              >
                <div className="w-10 h-10 rounded-xl bg-white dark:bg-[#141414] border border-[#E3E9F1] dark:border-[#2A2A2A] flex items-center justify-center text-[#2563EB] dark:text-[#3B82F6] shrink-0 shadow-2xs">
                  <i className="bi bi-chat-dots text-base"></i>
                </div>
                <div className="min-w-0">
                  <h4 className="text-xs font-bold text-[#0F172A] dark:text-white group-hover:text-[#2563EB] dark:group-hover:text-[#3B82F6] truncate">Chat Tutorial</h4>
                  <p className="text-[11px] text-[#64748B] dark:text-[#A3A3A3] mt-0.5 leading-tight">Socratic 1-on-1 tutor</p>
                </div>
              </button>
              ...
              >
                ...
                <div className="min-w-0">
                  <h4 className="text-xs font-bold text-[#0F172A] dark:text-white group-hover:text-[#2563EB] dark:group-hover:text-[#3B82F6] truncate">Flashcards</h4>
                  <p className="text-[11px] text-[#64748B] dark:text-[#A3A3A3] mt-0.5 leading-tight">Interactive 3D cards</p>
                </div>
              </button>
              ...
              >
                ...
                <div className="min-w-0">
                  <h4 className="text-xs font-bold text-[#0F172A] dark:text-white group-hover:text-[#2563EB] dark:group-hover:text-[#3B82F6] truncate">Quiz Test</h4>
                  <p className="text-[11px] text-[#64748B] dark:text-[#A3A3A3] mt-0.5 leading-tight">Timed test & timer</p>
                </div>
              </button>
              ...
              <button
                ...
                disabled={isLoadingContent}
                className="flex items-center gap-3 p-3.5 bg-[#F6F6F3] dark:bg-[#1C1C1C] hover:bg-[#F1F5F9] dark:hover:bg-[#2A2A2A] border border-[#E3E9F1] dark:border-[#2A2A2A] hover:border-[#2563EB]/50 dark:hover:border-[#3B82F6]/50 rounded-2xl text-left transition-all cursor-pointer group relative"
              >
                <div className="w-10 h-10 rounded-xl bg-white dark:bg-[#141414] border border-[#E3E9F1] dark:border-[#2A2A2A] flex items-center justify-center text-[#2563EB] dark:text-[#3B82F6] shrink-0 shadow-2xs">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1">
                    <h4 className="text-xs font-bold text-[#0F172A] dark:text-white group-hover:text-[#2563EB] dark:group-hover:text-[#3B82F6] truncate">Voice Tutorial</h4>
                    {!liveAccess.allowed && (
                  ...
                  <p className="text-[11px] text-[#64748B] dark:text-[#A3A3A3] mt-0.5 leading-tight">Blackboard voice lesson</p>
                </div>
              </button>

              {/* 5. Infographics (Coming Soon) */}
              <div className="flex items-center gap-3 p-3.5 bg-[#F8FAFC] dark:bg-[#1C1C1C]/50 border border-[#E2E8F0] dark:border-[#2A2A2A] rounded-2xl text-left opacity-80 relative">
                <div className="w-10 h-10 rounded-xl bg-white dark:bg-[#141414] border border-[#E3E9F1] dark:border-[#2A2A2A] flex items-center justify-center text-[#64748B] dark:text-[#A3A3A3] shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-bold px-1.5 py-0.5 bg-[#E2E8F0] dark:bg-[#2A2A2A] text-[#475569] dark:text-slate-300 rounded-full shrink-0">
                      Soon
                    </span>
                  </div>
                  <p className="text-[11px] text-[#64748B] dark:text-[#A3A3A3] mt-0.5 leading-tight">Visual concept summary</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-3.5 bg-[#F8FAFC] dark:bg-[#1C1C1C]/50 border border-[#E2E8F0] dark:border-[#2A2A2A] rounded-2xl text-left opacity-80 relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-[#64748B] dark:text-[#A3A3A3] mt-0.5 leading-tight">Audio overview episode</p>
                </div>
              </div>
            </div>
          </div>
