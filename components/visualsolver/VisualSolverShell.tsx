    ...
    if (typeof input === 'string') {
        ...
        if (input.startsWith('blob:')) {
            try {
                ...
                console.warn('Unable to read blob URL, it may have expired:', blobErr);
                throw new Error("Image preview expired. Please select or capture the photo again.");
            }
        }
        ...
        const checkSharedImageIntent = async () => {
            ...
            if (sharedImage) {
                try {
                    ...
                    if (sharedImage.startsWith('content://') || sharedImage.startsWith('file://') || sharedImage.startsWith('blob:')) {
                    ...
                    addToast("Could not load image. Please capture or upload again.", "error");
                    initializeCamera();
                }
            } else {
                initializeCamera();
                    ...
                    <div className="relative w-full h-full flex flex-col items-center justify-center bg-slate-950">
                        {scannedImage && (
                            <img
                                ...
                                onError={() => {
                                    console.warn('Scanned image preview failed to load');
                                    addToast('Image could not be loaded. Please retake photo.', 'error');
                                    handleRetake();
                                }}
                            />
                        )}
