...
/** Build a minimal profile when the user is not yet in allUsers */
...
export const MessengerNewChat: React.FC<MessengerNewChatProps> = ({
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (levelFilter !== 'all') {
      users = users.filter(
        (u) => String(u.level) === levelFilter || String(u.level) === `${levelFilter}L`
      );
    }
    ...
    return users;
  }, [allUsers, currentUser, scopeFilter, schoolFilter, levelFilter, newChatSearchQuery]);

  // Incoming / sent from partnerRequests — works even if user not in allUsers yet
  const receivedRequests = useMemo(() => {
    const entries = Object.entries(partnerRequests || {});
    return entries
      .filter(([, req]) => req && req.status === 'received')
      .map(([key, req]) => ({
        key,
        req,
        user: profileFromRequest(req, allUsers || [], 'incoming'),
      }));
  ...
  const sentRequests = useMemo(() => {
    const entries = Object.entries(partnerRequests || {});
    return entries
      .filter(([, req]) => req && req.status === 'sent')
      .map(([key, req]) => ({
        key,
        req,
        user: profileFromRequest(req, allUsers || [], 'sent'),
      }));
  }, [partnerRequests, allUsers]);

  const requestCount = receivedRequests.length + sentRequests.length;
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#F8F9FA] dark:bg-black text-neutral-900 dark:text-white overflow-hidden animate-fade-in">
      ...
      {/* Tabs: Find Partners | Requests */}
      <div className="flex border-b border-[#E9ECEF] dark:border-white/10 bg-white dark:bg-black px-4 py-3 shrink-0 gap-3">
        <button
          type="button"
          onClick={() => setActiveTab('find')}
          className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition cursor-pointer ${
            activeTab === 'find'
              ? 'bg-[#009EE2] text-white'
              : 'text-[#6C757D] dark:text-gray-400 hover:text-[#212529] dark:hover:text-white bg-neutral-50 dark:bg-black hover:bg-neutral-100 dark:hover:bg-neutral-900 dark:border dark:border-white/10'
          }`}
        >
          Find Partners
        </button>
        <button
          ...
          className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition flex items-center gap-2 cursor-pointer ${
            activeTab === 'requests'
              ? 'bg-[#009EE2] text-white'
              : 'text-[#6C757D] dark:text-gray-400 hover:text-[#212529] dark:hover:text-white bg-neutral-50 dark:bg-black hover:bg-neutral-100 dark:hover:bg-neutral-900 dark:border dark:border-white/10'
          }`}
        >
          ...
          {requestCount > 0 && (
            <span
              className={`rounded-full text-[10px] font-black h-5 min-w-5 px-1.5 flex items-center justify-center ${
                activeTab === 'requests'
                  ? 'bg-white text-[#009EE2]'
        ...
        {activeTab === 'find' ? (
          ...
          <div className="max-w-2xl mx-auto space-y-8">
            {receivedRequests.length > 0 && (
              <div>
                <h3 className="text-xs font-black uppercase tracking-wider text-[#6C757D] dark:text-gray-400 mb-4 pl-1">
                  Received Requests ({receivedRequests.length})
                </h3>
                <div className="space-y-2.5">
                  {receivedRequests.map(({ key, user }) => (
                    <div
                      key={key}
                      className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 sm:p-4 bg-white dark:bg-[#0A0A0A] border border-slate-200/80 dark:border-white/10 rounded-2xl"
                    >
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <Avatar
                          className="w-11 h-11 rounded-full shrink-0 object-cover border border-slate-200 dark:border-white/10"
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          type="button"
                          onClick={() => declinePartnerRequest(user)}
                          className="text-xs font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 px-3 py-1.5 rounded-xl transition cursor-pointer"
                        >
                          Decline
                        </button>
                        <button
                          type="button"
                          onClick={() => acceptPartnerRequest(user)}
                          className="text-xs font-bold text-white bg-[#0066FF] hover:bg-[#0055D4] px-3 py-1.5 rounded-xl transition cursor-pointer"
                        >
                          Accept
                        </button>
                  ...
                  {sentRequests.map(({ key, user }) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => declinePartnerRequest(user)}
                        className="text-xs font-bold text-red-500 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/40 px-3 py-1.5 rounded-xl hover:bg-red-100 dark:hover:bg-red-950/50 transition cursor-pointer shrink-0"
                      >
                        Cancel
                      </button>
                    </div>
            ...
            {receivedRequests.length === 0 && sentRequests.length === 0 && (
              <div className="text-center py-10 bg-white dark:bg-[#0A0A0A] border border-slate-200 dark:border-white/10 rounded-2xl p-6">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  When someone sends you a request, or you send one, it will show up here.
                </p>
                <button
                  type="button"
                  onClick={() => setActiveTab('find')}
                  className="mt-4 text-xs font-bold text-[#0066FF] hover:underline cursor-pointer"
                >
                  Find partners to connect with →
                </button>
              </div>
            )}
          </div>
