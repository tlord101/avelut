...
export function sanitizeSvg(svgString: string | null | undefined): string | null {
  ...
  // Fix unescaped ampersands in text or attributes (e.g. & instead of &amp;) to prevent xmlParseEntityRef errors
  cleanSvg = cleanSvg.replace(/&(?!amp;|lt;|gt;|quot;|apos;|#[0-9]+;|#x[0-9a-fA-F]+;)/gi, '&amp;');
  ...
  if (typeof window !== 'undefined' && typeof window.DOMParser !== 'undefined') {
    try {
      ...
      if (parserError) {
        // Fallback to HTML DOM parser which is forgiving of non-strict XML (e.g. entity refs, unescaped chars)
        const htmlDoc = parser.parseFromString(cleanSvg, 'text/html');
        const fallbackSvg = htmlDoc.querySelector('svg');
        if (fallbackSvg) {
          doc = parser.parseFromString(fallbackSvg.outerHTML, 'image/svg+xml');
          parserError = doc.querySelector('parsererror');
        }
      }

      if (parserError) {
        console.warn('[SVG Sanitizer] Unrecoverable SVG parse error, using regex sanitizer fallback:', parserError.textContent);
      } else {
